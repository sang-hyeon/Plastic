...
현재 우리는 프로그래밍 역사에 의해 Domain을 중심으로 설계하고, 이것을 위한 세부 사항들을 외부 종속석으로 여기는 것이 매우 당연하다는 것을 깨달았습니다. DDD, Clean Architecture와 같은 많은 Application 설계 방법론들이 모두 그것을 이야기합니다. 이것에 대해서는 이미 많은 자료들이 있기에 더 깊은 이야기를 하지 않겠습니다.

Plastic은 언급된 내용 Domain Layer를 노출하는 방법을 도우려고합니다.

![API Layer](../resources/ap_architecture.jpg)

여러분들은 이미 Application의 가장 중심에 있을 Domain Layer를 Meditor Pattern 혹은 ~~~Service와 같은 것들로 Wrapping 했을 수 있습니다. (위 그림에서는 API Layer에 속합니다.)

Plastic은 이 API Layer에 해당하며 이것을 위한 2가지를 제공합니다.

첫번째로는 Command 단위 노출입니다.
허나, 이 사항은 이미 CQRS를 추종하는 다양한 라이브러리에서 접할 수 있기에 특별하지는 않습니다.

그럼에도 하나 꼽자면 `커맨드에 대한 유효성 검사 지원 (CanExecute)`이 있습니다.

두번째는 Command 단위 노출을 위한 `소스 코드 자동화`입니다.

Plastic이 제안하는 방법으로 Application Command를 작성한다면... 이후 많은 소스 코드들이 자동 생성됩니다.
여기에는 IOC 컨테이너에 대한 등록, Class 선언, Pipeline 빌드 등등이 포함됩니다.

또한 생성되는 소스 코드들은 단위 테스트 할 필요가 없습니다. 해당 생성기는 이미 단위 테스트 되었기 때문입니다.

> 이후 많은 업데이트를 통해 더욱 더 다양한 소스 코드를 자동화 시킬 예정입니다.
...
> 생성된 Command에는 전역 파이프라인이 포함된 Source code가 구성됩니다.
